...
function applyTextcomplete($btnGroup) {
  ...
  var searchTimeout
  var actionResultTemplate = Handlebars.compile(
    $('#action-result-template').html()
  )
  ...
  $input.keyup(function() {
    ...
    if (
      searchType == 'tags' &&
      !$('a[data-value="' + inputValue + '"]').length
    ) {
      $divider.after(
        actionResultTemplate({ value: inputValue, suggestion: true })
      )
    }
  })
  ...
  var assignConversation = function($anchor) {
    var account = $("[name='account-slug']").val()
    var conversation = $("[name='conversation-number']").val()
    var assignConversationPath =
      '/' + account + '/' + conversation + '/assignee'
    var assignmentEventTemplate = Handlebars.compile(
      $('#assignment-event-template').html()
    )
    ...
    $.post(
      ...
      function(response) {
        $('.conversation-stream').append(
          assignmentEventTemplate(response.assignment_event)
        )
        resetCommandBar()
      },
      'json'
  ...
  var useCannedResponse = function($anchor) {
    ...
    var account = $("[name='account-slug']").val()
    var cannedResponsePath =
      '/' + account + '/canned_responses/' + $anchor.attr('data-id')

    $.getJSON(cannedResponsePath, function(response) {
...
$(function() {
  ...
  $(document).on('click', '#message_attachments_atributes_trigger', function() {
    ...
    $('#message_attachments_atributes').change(function() {
      $('#message_attachments_atributes_trigger')
        .find('span.counter')
        .html($('#message_attachments_atributes').get(0).files.length)
    })
